...
export default function WalletTransferDialog({ open, onOpenChange }: WalletTransferDialogProps) {
  const { transferFunds, balance } = useWallet();
  const { user } = useAuth();
  ...
  const handleTransfer = async () => {
    ...
    if (!selectedUser) {
      toast({
        ...
        description: 'Please search and select a user to transfer to',
        variant: 'destructive'
      });
      return;
    }
    ...
    const result = await transferFunds(
      selectedUser.id, 
      numAmount, 
      description || `Transfer to ${selectedUser.display_name || selectedUser.email}`
    );
    setProcessing(false);
    ...
    if (result.success) {
      toast({
        ...
        description: `$${numAmount.toFixed(2)} sent to ${selectedUser.display_name || selectedUser.email}`
      });
      onOpenChange(false);
    } else {
      toast({
